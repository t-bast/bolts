...
a "blinded" node ID and an encrypted blob (`encrypted_recipient_data`) for each
one of the blinded nodes.
...
The `encrypted_recipient_data` is a TLV stream, encrypted for a given blinded
node, that may contain the following TLV fields:
...
- MUST provide the `encrypted_recipient_data` field for each node in the blinded
  route in their respective onion payloads
